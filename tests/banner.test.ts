...
describe('renderBannerLines · 渐变模式', () => {
   ...
   * ⚠️ 整幅逐字比对，不是抽查。
   *
   * 字体是**裁过的子集**（`src/assets/ansi-shadow-subset.ts`），只留了 `TODO-VUE`
   * 用到的那几个字形，其余全换成空字形。于是「改了 `BRAND_NAME` 却没同步子集」
   * 这种错**不会报错、也不会变窄到触发别的断言**——新字母安静地渲染成一片空白，
   * 上面那条「含有 █」和下面那条宽度守卫都照样绿。只有整幅比对能拦住它。
   *
   * 期望值是拿**未裁剪的原始字体**渲染出来的，与被测的子集不同源。
   */
  it('字标与原始 ANSI Shadow 字体渲染的结果逐字相同', () => {
    expect(plain(lines()[1]).split('\n')).toEqual([
      '████████╗ ██████╗ ██████╗  ██████╗       ██╗   ██╗██╗   ██╗███████╗',
      '╚══██╔══╝██╔═══██╗██╔══██╗██╔═══██╗      ██║   ██║██║   ██║██╔════╝',
      '   ██║   ██║   ██║██║  ██║██║   ██║█████╗██║   ██║██║   ██║█████╗  ',
      '   ██║   ██║   ██║██║  ██║██║   ██║╚════╝╚██╗ ██╔╝██║   ██║██╔══╝  ',
